...
public class EffectSampler3 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Transform destination = myTransform;
            destination.rotation = Quaternion.Euler(0, -45, 0);
            StartCoroutine(EffectGenerater(destination));
        }
    }
    private IEnumerator EffectGenerater(Transform destination)
    {
        isAnimation = true;
    ...
    private IEnumerator ArrowManage(GameObject arrow)
    {
        yield return new WaitForSeconds(2.99f);
        Destroy(arrow);
    }
    private IEnumerator ArrowManage2(GameObject arrow)
    {
